...
function updateLabelsList() {
    ...
    if (state.labels.length === 0) {
        labelsContainer.innerHTML = '<span class="text-muted small">No labels added yet. Add at least 2 labels to classify.</span>';
        return;
    }

    labelsContainer.innerHTML = state.labels.map((label, index) => `
...
function initializeButtons() {
    document.getElementById('classifyBtn').addEventListener('click', classifyImages);
...
function displayResults() {
    ...
    Object.entries(groupedResults).forEach(([label, files]) => {
        ...
        files.forEach(file => {
            ...
            const scoreClass = result.top_score >= 80 ? 'score-high' : 
                              result.top_score >= 50 ? 'score-medium' : 'score-low';
...
function downloadResults() {
    ...
    if (state.results instanceof Blob) {
        ...
        const extension = state.exportFormat === 'excel' ? 'xlsx' : state.exportFormat;
